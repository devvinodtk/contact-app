...
import {
  MessageTemplates, MessageParams, MessagePayload, Component,
...
export const sendUserSignUpWhatsAppMessage = onCall(async (request) => {
...
export const sendMemberVerifiedWhatsAppMessage = onCall(async (request) => {
  ...
  const recipientPhoneNumber = phoneNumber.replace(/\D/g, '');
  const payload = constructMessagePayload(
    MessageTemplates.member_verified_dev,
    { memberName, phoneNumber: recipientPhoneNumber, profileLink },
  );
